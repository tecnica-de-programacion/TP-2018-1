...
#        raw_text.append(self.end)
#        tale = self.separator.join(raw_text)
#        return "{} {}".format(self.init, tale)
#
#print("***" * 5, "House", "***" * 5)
#jack_house = House()
...
#
#class EcoHouse(House):
#    
#    def __init__(self):
#        super().__init__()
#        self.lines = list(map(lambda x: x + " " + x, self.lines))
#
#print("***" * 5, "Eco House", "***" * 5)
...
class House:
    ...
    def __init__(self, data, order=DefaulrOrder(), repeat=DefaulrRepeat()):
        space_len = len(self.data["init"]) + 1
        spaces = " " * space_len
        self.separator = "\n" + spaces
